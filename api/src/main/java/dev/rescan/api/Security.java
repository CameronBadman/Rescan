...
public class Security {
    @Bean JwtDecoder jwtDecoder() {
        ...
        OAuth2TokenValidator<Jwt> clientValidator = jwt ->
            client.equals(jwt.getClaimAsString("client_id")) && "access".equals(jwt.getClaimAsString("token_use")) && jwt.getSubject() != null
                ? OAuth2TokenValidatorResult.success()
                : OAuth2TokenValidatorResult.failure(new OAuth2Error("invalid_token"));
        decoder.setJwtValidator(new DelegatingOAuth2TokenValidator<>(JwtValidators.createDefaultWithIssuer(issuer), clientValidator));
        return decoder;
    }
