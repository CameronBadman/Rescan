...
public class JobsController {
  ...
  public Map<String, Object> list(
      ...
      @RequestParam(defaultValue = START) UUID after,
    ...
    if (!status.isEmpty()
        && !Set.of(
                ...
                "FAILED",
                "DELETING")
            .contains(status)) throw new IllegalArgumentException("Invalid status");
    return page(store.list(store.user(jwt.getSubject()), after, status, limit + 1), limit);
  }

  @GetMapping("/{id}")
  public Map<String, Object> detail(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID id) {
  ...
  public Map<String, Object> documents(
      ...
      @RequestParam(defaultValue = "50") int limit) {
    pageSize(limit);
    return page(store.documents(store.user(jwt.getSubject()), id, after, limit + 1), limit);
  }
}
