...
public class Errors {
  @ExceptionHandler(JobStore.MissingJob.class)
  ...
  ResponseEntity<?> invalid(IllegalArgumentException error) {
    return ResponseEntity.badRequest()
        .body(Map.of("code", "INVALID_REQUEST", "message", error.getMessage()));
  }

  @ExceptionHandler(Conflict.class)
  ResponseEntity<?> conflict(Conflict error) {
    return ResponseEntity.status(409)
        .body(Map.of("code", "CONFLICT", "message", error.getMessage()));
  }
