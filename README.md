...
**GPUs on RunPod.** One pod, **Qwen3.8-27B** on an H100, serving every pass.
Two ways to get it: `./scripts/runpod_up.sh` uses `runpodctl` — no state,
on-demand, writes the `RESCAN_LLM_*` lines into `.env` and
`infra/aws/llm.auto.tfvars`; `./scripts/runpod_down.sh` deletes the pod and
keeps the weight volume. `infra/runpod/` is the Terraform equivalent with
spot pricing: `terraform apply` for a batch, `terraform destroy` after. Both
can optionally add a second pod running a 235B MoE for the once-per-job
plan-compile pass (`RESCAN_COMPILE_*`; the client routes `compile_dsl` there
automatically) — only worth it if the 27B compiles plans badly. See
