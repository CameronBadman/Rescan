...
public class DocumentParser {
    ...
    public Map<String,Object> parse(Path source,String filename,UUID job,UUID doc) throws Exception {
        ...
        if(type.equals("application/octet-stream") || Set.of("application/zip","application/gzip","application/x-tar","application/x-7z-compressed","application/x-rar-compressed","application/x-executable").contains(type)) throw new ParseFailure("UNSUPPORTED_FORMAT");
    ...
    protected Map<String,Object> image(Path source) throws Exception { return vision(source); }
    protected Map<String,Object> vision(Path source) throws Exception {
        ...
        var process=new ProcessBuilder(Settings.get("PYTHON_BIN","python3"),Settings.get("VIT_SCRIPT","worker/python/recognize.py"),source.toString(),output.toString())
            .redirectOutput(ProcessBuilder.Redirect.DISCARD).redirectError(ProcessBuilder.Redirect.DISCARD).start();
        if(!process.waitFor(Settings.integer("DOCUMENT_TIMEOUT_SECONDS",900),java.util.concurrent.TimeUnit.SECONDS)) { WorkerMain.kill(process); throw new ParseFailure("TIMEOUT"); }
        if(process.exitValue()!=0) throw new ParseFailure("OCR_FAILED");
        try { return new ObjectMapper().readValue(output.toFile(),new com.fasterxml.jackson.core.type.TypeReference<Map<String,Object>>() {}); }
    ...
    protected Map<String,Object> pdf(Path source,Metadata metadata) throws Exception {
        ...
        try(var document=Loader.loadPDF(source.toFile())) {
            ...
            if(document.getNumberOfPages()>Settings.integer("MAX_PAGES",50)) throw new ParseFailure("PAGE_LIMIT");
            ...
            for(int i=0;i<document.getNumberOfPages();i++) {
                ...
                if(!stripper.getText(document).isBlank()) {
                    ...
                    try(var pageDoc=new PDDocument()) { pageDoc.importPage(document.getPage(i)); pageDoc.save(single.toFile()); }
                    var parsed=tika(single,metadata); Files.delete(single);
                    pages.put(i+1,Map.of("page",i+1,"text",parsed.get("text"),"method","TIKA")); nativeText=true;
                } else {
                    var box=document.getPage(i).getCropBox();
                    double pixels=box.getWidth()*box.getHeight()*Math.pow(200.0/72,2);
                    if(pixels>Settings.integer("MAX_PAGE_PIXELS",12000000)) throw new ParseFailure("IMAGE_LIMIT");
                    ImageIO.write(renderer.renderImageWithDPI(i,200),"PNG",images.resolve(String.format("%04d.png",i+1)).toFile()); ocr=true;
                }
            }
            Map<String,Object> recognized=ocr ? vision(images) : Map.of();
            if(ocr) {
                @SuppressWarnings("unchecked") var scanned=(List<Map<String,Object>>)recognized.get("pages");
            ...
            result.put("pages",new ArrayList<>(pages.values())); result.put("ocrUsed",ocr);
            result.put("extractionMethod",ocr ? (nativeText?"TIKA_VIT":"VIT") : "TIKA");
            result.put("warnings",List.of()); if(ocr) result.put("modelRevision",recognized.get("modelRevision"));
            return result;
        } finally { try(var paths=Files.walk(images)) { for(var path:paths.sorted(Comparator.reverseOrder()).toList()) Files.deleteIfExists(path); } }
    }
