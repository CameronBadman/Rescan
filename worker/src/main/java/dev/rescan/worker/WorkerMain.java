...
public class WorkerMain {
    ...
    public static void main(String[] args) throws Exception {
        if(args.length>0 && args[0].equals("--parse")) { DocumentParser.child(args); return; }
        Runtime.getRuntime().addShutdownHook(new Thread(() -> { RUNNING.set(false); kill(CHILD.get()); }));
        try(var context=new AnnotationConfigApplicationContext(Infrastructure.class); var blobs=new BlobStore(); var queue=new Queue()) {
            ...
            while(RUNNING.get()) {
                ...
                } catch(Exception e) { LOG.log(System.Logger.Level.WARNING,"Worker iteration failed: {0}",e.getClass().getSimpleName()); Thread.sleep(1000); }
            }
        }
    }
    private static void process(WorkStore.Claim c,WorkStore work,BlobStore blobs,Queue queue) throws Exception {
        ...
        try(var heartbeat=Executors.newSingleThreadScheduledExecutor()) {
            ...
            heartbeat.scheduleAtFixedRate(() -> {
                ...
                catch(Exception e) { alive.set(false); kill(CHILD.get()); }
            },20,20,TimeUnit.SECONDS);
            Path input=temp.resolve("input"),output=temp.resolve("output.json");
            blobs.download(c.sourceKey(),c.sourceVersion(),input);
            if(Files.size(input)!=c.bytes() || Files.size(input)>Settings.maxFileBytes()) throw new DocumentParser.ParseFailure("SIZE_MISMATCH");
            var builder=new ProcessBuilder(Path.of(System.getProperty("java.home"),"bin","java").toString(),"-Xmx2g","-Djava.io.tmpdir="+temp,
                "-cp",System.getProperty("java.class.path"),WorkerMain.class.getName(),"--parse",input.toString(),output.toString(),c.filename(),c.jobId().toString(),c.id().toString());
            // The parser does not need credentials, service discovery, or application secrets.
            var env=builder.environment(); var kept=new HashMap<String,String>();
            for(String key:List.of("PATH","LANG","PYTHON_BIN","VIT_SCRIPT","VIT_MODEL_DIR","DETECTOR_MODEL_DIR","MAX_PAGES","MAX_PAGE_PIXELS")) if(env.containsKey(key)) kept.put(key,env.get(key));
            env.clear(); env.putAll(kept); env.put("HF_HUB_OFFLINE","1"); env.put("TRANSFORMERS_OFFLINE","1");
            builder.redirectOutput(ProcessBuilder.Redirect.DISCARD).redirectError(ProcessBuilder.Redirect.DISCARD);
            Process child=builder.start(); CHILD.set(child);
            if(!child.waitFor(Settings.integer("DOCUMENT_TIMEOUT_SECONDS",900),TimeUnit.SECONDS)) { kill(child); throw new DocumentParser.ParseFailure("TIMEOUT"); }
            CHILD.compareAndSet(child,null);
            if(!alive.get() || !RUNNING.get()) return;
            if(child.exitValue()!=0) {
                String code=Files.exists(output) ? new ObjectMapper().readTree(output.toFile()).path("code").asText("PARSER_FAILED") : "PARSER_FAILED";
            ...
            boolean terminal=work.fail(c,e.code,Set.of("TIMEOUT","PARSER_FAILED","OCR_FAILED").contains(e.code));
