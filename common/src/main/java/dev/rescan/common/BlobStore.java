...
public class BlobStore implements AutoCloseable {
    ...
    public BlobStore() {
        ...
        var config = S3Configuration.builder().pathStyleAccessEnabled(!Settings.get("S3_ENDPOINT", "").isBlank()).build();
        ...
        client = cb.build(); signer = sb.build();
    }
    public Map<String,Object> upload(UUID id, String key, long bytes) {
        var signed = signer.presignPutObject(p -> p.signatureDuration(Duration.ofMinutes(15)).putObjectRequest(r ->
            r.bucket(bucket).key(key).contentLength(bytes).ifNoneMatch("*")));
        var headers = new TreeMap<>(signed.signedHeaders());
        headers.remove("host"); headers.remove("content-length");
        return Map.of("documentId",id,"url",signed.url().toString(),"method","PUT","headers",headers,"expiresInSeconds",900);
    }
    public HeadObjectResponse head(String key) { return client.headObject(r -> r.bucket(bucket).key(key)); }
    ...
    public void putJson(String key, Path source) {
        client.putObject(r -> r.bucket(bucket).key(key).contentType("application/json"), RequestBody.fromFile(source));
    }
    public String resultUrl(String key) {
        return signer.presignGetObject(r -> r.signatureDuration(Duration.ofMinutes(5)).getObjectRequest(g ->
            g.bucket(bucket).key(key).responseContentType("application/json"))).url().toString();
    }
    public void deletePrefix(String prefix) {
        ...
        for (var page : client.listObjectVersionsPaginator(r -> r.bucket(bucket).prefix(prefix))) {
            ...
            page.deleteMarkers().forEach(v -> ids.add(ObjectIdentifier.builder().key(v.key()).versionId(v.versionId()).build()));
