...
public class BlobStore implements AutoCloseable {
  ...
  public BlobStore() {
    ...
    cb.overrideConfiguration(
        c ->
            c.apiCallTimeout(Duration.ofSeconds(20)).apiCallAttemptTimeout(Duration.ofSeconds(10)));
    ...
    if (!Settings.get("S3_ENDPOINT", "").isBlank()) {
      ...
      sb.endpointOverride(
          URI.create(Settings.get("S3_PUBLIC_ENDPOINT", Settings.require("S3_ENDPOINT"))));
    }
    client = cb.build();
    signer = sb.build();
  }
