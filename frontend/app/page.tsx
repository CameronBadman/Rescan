...
function describeEvent(entry: AuditEntry): { title: string; desc: string } {
  ...
  switch (entry.event) {
    case 'job_created': return { title: 'Hiring round created', desc: `${d.accepted ?? 0} documents accepted${d.duplicates ? `, ${d.duplicates} duplicates skipped` : ''} for ${d.role ?? 'the role'}` }
    ...
    case 'job_complete': return { title: 'Round complete', desc: `${d.shortlisted} shortlisted, ${d.excluded} excluded with reasons` }
...
export default function Page() {
  ...
  // Poll the job while it runs; load its results once it settles.
  useEffect(() => {
    if (!jobId) return
    let cancelled = false
    ...
    const tick = async () => {
      try {
        const [detail, current] = await Promise.all([api.job(jobId), api.status(jobId)])
        if (cancelled) return
        setJob(detail); setStatus(current)
        if (settled(current.status)) {
          const [sl, rs, au] = await Promise.all([
            detail.has_shortlist ? api.shortlist(jobId) : Promise.resolve(null),
            detail.has_rules ? api.rules(jobId) : Promise.resolve(null),
            api.audit(jobId),
          ])
          if (cancelled) return
  ...
  const roleTitle = job?.role?.title ?? shortlist?.role_title ?? '—'
  ...
  const review = async (ref: string) => {
    if (!jobId || identities[ref] !== undefined) return
    const identified = await api.shortlist(jobId, true)
    const all = [...identified.entries, ...identified.below_cutoff, ...identified.excluded, ...identified.manual_review]
    const found = all.find((e) => e.candidate_ref === ref)
    setIdentities((current) => ({ ...current, [ref]: found?.identity ?? null }))
  }
  ...
  return <main className="min-h-screen bg-[#f7f8f6] text-[#19312b]">
    <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col border-r border-[#dfe7e1] bg-[#fbfcfa] px-5 py-6 lg:flex">
      ...
      <nav className="mt-3 flex flex-col gap-1">{navItems.map(([label, Icon]) => <button key={label} onClick={() => go(label)} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition ${activeNav === label ? 'bg-[#e8f0ea] font-semibold' : 'text-[#6f7e76] hover:bg-[#f0f4f0]'}`}><Icon size={17} strokeWidth={1.8} /><span>{label}</span>{label === 'Candidates' && status && <span className="ml-auto rounded-full bg-[#dce9df] px-2 py-0.5 text-[10px] font-bold">{status.total}</span>}</button>)}</nav>
      <div className="mt-auto rounded-xl border border-[#dfe7e1] bg-[#f1f6f1] p-4"><div className="flex items-center gap-2 text-xs font-semibold"><LockKeyhole size={14} /> Privacy-first by design</div><p className="mt-2 text-xs leading-5 text-[#73827a]">Identities stay hidden until a human makes the shortlist.</p><a href={`${API_URL}/docs`} target="_blank" rel="noreferrer" className="mt-3 text-xs font-bold text-[#47765f]">API reference <ArrowUpRight className="ml-1 inline" size={13} /></a></div>
      <div className="mt-5 flex items-center gap-3 border-t border-[#dfe7e1] pt-5"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e9c7a9] text-xs font-bold">RT</div><div><p className="text-xs font-bold">Recruiting team</p><p className="text-[11px] text-[#839088]">{jobs.length} hiring rounds</p></div><MoreHorizontal className="ml-auto text-[#9aa59e]" size={17} /></div>
    </aside>
    <section className="lg:ml-64">
      <header className="flex items-center justify-between border-b border-[#dfe7e1] bg-[#fbfcfa] px-6 py-5 md:px-10"><div><p className="text-xs font-semibold text-[#829088]">{new Date().toLocaleDateString('en-AU', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p><h1 className="mt-1 text-2xl font-semibold tracking-tight">Good {new Date().getHours() < 12 ? 'morning' : 'afternoon'}</h1></div><div className="relative flex items-center gap-3"><button aria-label="Help" className="rounded-full p-2 text-[#718078] hover:bg-[#eef3ee]"><CircleHelp size={19} /></button><button onClick={() => setShowJobs((v) => !v)} className="flex items-center gap-2 rounded-lg border border-[#dce5de] bg-white px-3 py-2 text-xs font-semibold">{jobId ? `${roleTitle} / ${jobId}` : 'No hiring round'} <ChevronDown size={14} /></button>
        {showJobs && <div className="absolute right-0 top-12 z-10 w-72 rounded-lg border border-[#dfe7e1] bg-white p-2 shadow-lg">{jobs.length === 0 && <p className="p-2 text-xs text-[#819088]">No rounds yet.</p>}{jobs.map((j) => <button key={j.id} onClick={() => { setJobId(j.id); setShowJobs(false) }} className={`flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-xs hover:bg-[#f0f4f0] ${j.id === jobId ? 'font-bold' : ''}`}><span>{j.id}</span><span className="text-[#8a9890]">{j.status}</span></button>)}</div>}
      </div></header>
...
function Overview({ go, status, shortlist, rules, audit, running, roleTitle, jobId, onNew, onRuleAdded }: { go: (v: View) => void; status: JobStatus | null; shortlist: Shortlist | null; rules: RuleSet | null; audit: AuditEntry[]; running: boolean; roleTitle: string; jobId: string | null; onNew: () => void; onRuleAdded: () => void }) {
  ...
  const closerLook = (shortlist?.manual_review.length ?? 0) + (status?.counts?.needs_manual_review ?? 0)
  const tiles: [string, string, typeof Users, number][] = [
    [String(total), 'Applications received', Users, total ? 100 : 0],
    [String(processed), running ? 'Processed so far' : 'Anonymized & scored', Sparkles, total ? Math.round((processed / total) * 100) : 0],
    [String(shortlisted), 'Shortlisted', Check, total ? Math.round((shortlisted / total) * 100) : 0],
    [String(closerLook), 'Need a closer look', Info, total ? Math.round((closerLook / total) * 100) : 0],
  ]
  return <><PageHead eyebrow={running ? 'Hiring round in progress' : 'Active hiring round'} title={<>{roleTitle} <span className="text-[#8c9991]">/ {jobId ?? '—'}</span></>} body={running ? `Processing: ${processed} of ${total} documents through extraction, anonymization and screening.` : jobId ? 'A privacy-first workspace for explainable candidate review.' : 'Create a hiring round to begin.'} action={<NewRoundButton onClick={onNew} />} />
...
function CandidatesView({ rows, total, query, setQuery, identities, onReview, onNew }: { rows: Row[]; total: number; query: string; setQuery: (q: string) => void; identities: Record<string, Identity>; onReview: (ref: string) => void; onNew: () => void }) {
  return <><PageHead eyebrow="Candidate workspace" title="Candidates" body="Review anonymized applications, match evidence, and human decisions in one place." action={<NewRoundButton onClick={onNew} />} />
    <div className="rounded-xl border border-[#dfe7e1] bg-white p-6"><div className="flex flex-col justify-between gap-4 md:flex-row md:items-center"><div><h3 className="font-semibold">{total} anonymized applications</h3><p className="mt-1 text-xs text-[#819088]">Identities remain hidden until you open a candidate for review.</p></div><div className="flex gap-2"><div className="flex items-center gap-2 rounded-lg bg-[#f5f8f5] px-3 py-2.5"><Search size={16} className="text-[#91a099]" /><input value={query} onChange={(e) => setQuery(e.target.value)} aria-label="Search candidates" placeholder="Search candidates" className="w-48 bg-transparent text-xs outline-none" /></div><button className="rounded-lg border border-[#dfe7e1] px-3 text-xs font-semibold"><Filter size={14} /></button></div></div>
...
function AuditView({ audit, rules, status, reviewed, roleTitle, jobId }: { audit: AuditEntry[]; rules: RuleSet | null; status: JobStatus | null; reviewed: number; roleTitle: string; jobId: string | null }) {
  ...
  const events = Array.from(new Set(audit.map((e) => e.event))).sort()
  const shown = [...audit].reverse().filter((e) => filter === 'all' || e.event === filter).slice(0, 80)
  return <><PageHead eyebrow="Traceability" title="Audit trail" body="A reviewable record of every rule, score, and human decision in this hiring round." /><div className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]"><section className="rounded-xl border border-[#dfe7e1] bg-white p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">Decision history</h3><p className="mt-1 text-xs text-[#819088]">{audit.length} events for {roleTitle} / {jobId ?? '—'}.</p></div><label className="flex items-center gap-2 rounded-lg border border-[#dfe7e1] px-3 py-2 text-xs font-semibold"><Filter size={14} /><select value={filter} onChange={(e) => setFilter(e.target.value)} className="bg-transparent outline-none"><option value="all">All events</option>{events.map((ev) => <option key={ev} value={ev}>{ev.replace(/_/g, ' ')}</option>)}</select></label></div>
    <div className="mt-6 space-y-6">{shown.map((entry, i) => { const { title, desc } = describeEvent(entry); return <div key={entry.id} className="flex gap-4"><div className="flex flex-col items-center"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#e8f1e9] text-[#47765f]"><FileCheck2 size={16} /></div>{i < shown.length - 1 && <div className="mt-2 h-full w-px bg-[#e2ebe3]" />}</div><div className="pb-2"><p className="text-xs text-[#8a9890]">{timeAgo(entry.at)} · {entry.stage}</p><p className="mt-1 text-sm font-semibold">{title}</p><p className="mt-1 text-xs leading-5 text-[#718078]">{desc}</p>{entry.candidate_id && <p className="mt-2 text-[10px] font-semibold uppercase tracking-wider text-[#a0aba4]">{entry.detail?.candidate_ref ?? 'candidate'}</p>}</div></div> })}{shown.length === 0 && <p className="text-xs text-[#8a9890]">No events yet.</p>}</div></section>
    <section className="rounded-xl border border-[#dfe7e1] bg-[#f1f6f1] p-6"><div className="flex items-center gap-2"><LockKeyhole size={17} /><h3 className="font-semibold">Explainability record</h3></div><p className="mt-3 text-sm leading-6 text-[#66786d]">Every score links back to the evidence and validated rule that produced it. Identity data is unlocked only after a human review decision.</p><div className="mt-6 space-y-3">{[['Rules compiled', String(rules?.rules.length ?? 0)], ['Rules flagged', String(rules?.flagged ?? 0)], ['Profiles processed', String(status?.processed ?? 0)], ['Human reviews', String(reviewed)], ['Export', jobId ? 'GET /jobs/' + jobId + '/audit' : '—']].map(([a, b]) => <div className="flex justify-between border-b border-[#dfe7e1] pb-3 text-xs" key={a}><span className="text-[#819088]">{a}</span><span className="font-semibold">{b}</span></div>)}</div></section></div></>
}

function Activity({ audit }: { audit: AuditEntry[] }) {
  const icons: Record<string, typeof Layers3> = { plan_compiled: ShieldCheck, rule_risk_flagged: ShieldCheck, job_complete: FileCheck2, scored: Layers3, bucket_pulled: Upload, job_created: Upload }
  const interesting = [...audit].reverse().filter((e) => ['job_created', 'bucket_pulled', 'plan_compiled', 'rule_risk_flagged', 'ensemble_started', 'job_complete', 'query_run'].includes(e.event)).slice(0, 5)
...
function RoundModal({ onClose, onCreated }: { onClose: () => void; onCreated: (jobId: string) => void }) {
  ...
  return <div className="fixed inset-0 z-20 flex items-center justify-center bg-[#19312b]/35 p-4"><div role="dialog" aria-modal="true" className="w-full max-w-lg rounded-2xl border border-[#dfe7e1] bg-[#fbfcfa] p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#6c8175]">Workspace setup</p><h2 className="mt-2 text-2xl font-semibold">New hiring round</h2><p className="mt-2 text-sm leading-6 text-[#819088]">Describe the role in plain language. Requirements that screen on a protected attribute are flagged, never applied.</p></div><button onClick={onClose} aria-label="Close" className="rounded-full p-2 text-[#718078] hover:bg-[#eef3ee]"><X size={18} /></button></div>
    <div className="mt-6 space-y-4"><label className="block text-xs font-semibold">Role<input value={role} onChange={(e) => setRole(e.target.value)} className="mt-2 w-full rounded-lg border border-[#dfe7e1] bg-white px-3 py-2.5 text-sm outline-none focus:border-[#76a383]" /></label>
